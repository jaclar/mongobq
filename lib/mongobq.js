...
function MongoBQ(opts) {
  ...
  if (opts.map) {
    self._map = self._genMap(opts.map);
  }
  self.file = function () {
...
MongoBQ.prototype._genMap = function (map) {
    var res = {};

    Object.keys(map).forEach(function (key) {
      res[key] = {};
      res[key].getKey = function () {
        return map[key].name;
      };

      switch (map[key].type) {
      case "TIMESTAMP":
        res[key].transformValue = function (value) {
          // assumes value is js timestamp
          return value / 1000.0;
        };
        break;
      case "JSON":
...
MongoBQ.prototype.transformJSON = function (map) {
    return through2.obj(function (data, encoding, next) {
        ...
        Object.keys(data).forEach(function (key) {
            if (map[key]) {
                res[map[key].getKey()] = map[key].transformValue(data[key]);
            } else {
                res[key] = data[key];
            }
        });
...
MongoBQ.prototype.uploadToGCS = function (next) {
  ...
  var stream = self.mongo.stream(self.opts.batchSize)
                .pipe(self.transformJSON(self._map))
